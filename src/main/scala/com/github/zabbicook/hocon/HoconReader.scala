...
object HoconReads {
  ...
  implicit def enumProp[V, T <: EnumProp[V]](implicit s2enum: StringToEnumProp[V, T]): HoconReads[T] = {
    string.flatMap { s =>
      HoconReads.of { (config, meta) =>
        if (s2enum.acceptable(s)) {
          HoconSuccess(s2enum.convert(s))
        } else {
          HoconError.NotAcceptableValue(config.origin(), s, meta)
        }
      }
    }
  }
