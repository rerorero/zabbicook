...
class ZabbixApiSpec extends UnitSpec with OperationHelper with TestConfig {
  withTestApiConf { (apiConf, version) =>
    lazy val sut = new ZabbixApi(apiConf)

    version + "ZabbixApi.request" should "returns version info" in {
    ...
    version + "ZabbixApi.request" should "authorize" in {
      val q = Json.obj()
        .outExtend()
    ...
    version + "ZabbixApi.requestSingle" should "return single result" in {
      val q = Json.obj()
        .outExtend()
        .filter("alias" -> apiConf.authUser)
    ...
    version + "ZabbixApi.requestSingle" should "return none if not exists" in {
      val q = Json.obj()
        .outExtend()
    ...
    version + "ZabbixApi.requestSingle" should "throws an exception if duplicated" in {
