...
class UserOpSpec extends UnitSpec with TestUsers with TestConfig {
  withTestOp { (ops, version) =>
    lazy val sut = ops.user

    version + "present and absent" should "create, delete and update user" in {
    ...
    version + "presentPassword" should "change passwords" in {
      def clean() = {
        cleanTestUsers(ops)
      }
    ...
    version + "UserConfig" should "parsed from Hocon" in {
      val s = s"""{
                  |  user {
                  |    alias: "Alice"
                  |    lang: "en"
                  ...
                  |  initialPassword: false
                  |}""".stripMargin
      val actual = HoconReader.read[UserConfig](ConfigFactory.parseString(
        s), UserConfig.optional("root"))
      assert(
        actual === HoconSuccess(UserConfig(
        user = User(alias = "Alice", lang = Some(
          "en")),
