...
case class ErrorResponseException(method: String, response: ZabbixErrorResponse, cause: Throwable = null)
  extends ApiException(s"${method} responds error: ${response.getMessage}", cause)
