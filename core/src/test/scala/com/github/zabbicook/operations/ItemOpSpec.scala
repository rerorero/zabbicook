...
class ItemOpSpec extends UnitSpec with TestItems {
  ...
  "present" should "create, delete and update items in templates" in {
    ...
    cleanRun(clean) {
      assert(None === await(templateOp.findByHostname(appended._1)))

      // creates
      {
        presentTestItems()
        testItems.foreach { case (hostname, items) =>
          val Some(template) = await(templateOp.findByHostname(hostname))
      ...
      val Some(ts) = await(templateOp.findByHostname(appended._1))
