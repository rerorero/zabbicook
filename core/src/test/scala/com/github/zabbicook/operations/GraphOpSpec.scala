...
class GraphOpSpec extends UnitSpec with TestGraphs {
  ...
  withTestOp { (ops, version) =>
    ...
    version+"present" should "create, delete and update graph and graph items in templates" in {
      ...
      cleanRun(clean) {
        ...
        // create a graph
        {
          ...
          testGraphs.foreach { gs =>
            val Some(template) = await(ops.template.findByHostname(gs._1.template.host))
        ...
        val appendAt = await(ops.template.findByHostnamesAbsolutely(Seq(_appendAt.template.host)).map(_.head))
        assert(Seq() === await(
    ...
    version+"present" should "create, delete and update items in the graph setting" in {
      val itemAppend =
        GraphItemSetting(
          color = "DEAD00",
          itemName = item1.name,
      ...
      cleanRun(clean) {
        ...
        val template = await(ops.template.findByHostnamesAbsolutely(Seq(key.template.host)).map(_.head))
